...
llm_with_censor = GigaChat(model=model, timeout=600, profanity_check=True, temperature=1e-15)
...
class RouteQuery(BaseModel):
    """Инструмент для запроса дополнительных данных для ответа на вопрос польователя: 
vectorstore (векторное хранилище знаний),
web_search (поиск в интернете),
self_answer (дополнительные данные не требуются)"""
...
# Prompt
system = f"""Ты эксперт по маршрутизации пользовательских вопросов в базу данных (vectorstore),
веб-поиск (web_search) или ответь сам (self_answer)
{MAIN_KNOWLEDGE}
Ты должен принять решения, где взять данные для ответа на вопрос пользователя, 
если он касается технической поддержки или является техническим вопросом от разработчика.
Используй vectorstore для ответов на вопросы, связанные GigaChat, GigaChain, GigaChat API, 
GigaGraph, LangChain, LangGraph 
и другими техническими вопросами, которые могут быть связаны с работой с гигачатом, 
а также процессом подключения к нему, 
интеграцией, стоимостью, заключением договоров и т.п. а также использованием библиотеки 
gigachain для работы с гигачатом (gigachat) и 
другими большими языковыми моделями, эмбеддингами и т.д. Используй web_search 
ТОЛЬКО в случаях, когда вопрос пользователя очевидно 
не относится к GigaChat, LLM, AI, техническим проблемам с гигачатом, 
его АПИ, СДК, ключами, токенами и том подобным вещам.

Если вопрос пользователя простой или это вообще не вопрос, 
а утверждение или реплика или приветстиве или не понятно что, 
то используй self_answer. self_answer будет означать, 
что на такой вопрос GigaChat ответит самостоятельно без 
использования внешних данных.

Если вопрос пользователя выглдяит опасно или не относится 
к вопросам технической поддержки, просит поискать что-то в 
интернете, затрагивает чувствительные темы, 
относится к политике, религии, расизму и т.д., то используй self_answer. 
Ты не должен искать в интернете вопросы, 
которые не относятся к области технической 
поддержки пользователей. Если вопрос не относится к 
технической поддержке - выбирай self_answer.
"""
...
system = f"""Ты оцениваешь релевантность найденного документа по 
отношению к пользовательскому вопросу.
{MAIN_KNOWLEDGE}
...
async def generate(state):
    ...
    [
        (
            "system",
            f"""Ты - консультант технической поддержки по GigaChat и GigaChain.
Ты должен ответить на воспро пользователя. 
{MAIN_KNOWLEDGE}
...
# Define the nodes
workflow.add_node("🕵️‍♂️ Web Researcher", web_search)
workflow.add_node("👨‍💻 Documents Retriever", retrieve)
workflow.add_node("👨‍🔧 Document viewer", grade_documents)
workflow.add_node("🧑‍🎓 Consultant", generate)
workflow.add_node("👨‍🎨 Improviser", generate)
workflow.add_node("👷‍♂️ Query rewriter", transform_query)
workflow.add_node("👨‍⚖️ Finalizer", finalize)
